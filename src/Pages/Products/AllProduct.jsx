...
const AllProduct = () => {
    ...
    const handleFetch = async () => {
        try {
            const res = await axios.get('http://localhost:4000/api/getAllProducts');
            const reverseData = res.data.data
            console.log("allproduct",res.data.data)
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await axios.delete(`http://localhost:4000/api/deleteProductById/${id}`);
