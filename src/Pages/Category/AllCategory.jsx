    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await axios.delete(`http://localhost:4000/api/delete-category/${id}`);
                    ...
                    <tbody>
                        {currentItems.map((category, index) => (
                            <tr key={category._id}>
                                ...
                                <td><img src={category.CatImg} alt={category.categoryName} /></td>
                                {/* <td><input type="checkbox" checked={category.categoryActive ? true : false} readOnly /></td> */}
                                <td><Link to={`/update-category/${category._id}`} className="bt edit">Edit <i class="fa-solid fa-pen-to-square"></i></Link></td>
